<template>
  <v-container fluid class="d-flex justify-space-around">
    <v-container width="95%" height="200px">
      <v-card-title class="text-h6"> Invite Alumni Here </v-card-title>
    ...
    <v-container width="25%">
      <v-col cols="12">
        <v-card
          ...
          :to="{ path: '/card' }"
        >
          <v-card-title class="text-h4"> ALUMNI LOGIN </v-card-title>
      ...
      <v-col cols="12">
        <v-card color="#22BBEA" dark class="text-center">
          <v-card-title class="text-h4"> ALUMNI INVITE </v-card-title>
          <v-card-text class="text-h3">
  ...
  methods: {
    ...
    getuserInvite() {
      axios.get("/users").then((res) => {
        this.alumnisInvite = res.data.filter(
          (user) => user.first_name === null && user.last_name === null
        );
      });
    },
  },
